...
import com.wuziqi.love.fantasy.faith.player.AiFactory;
import com.wuziqi.love.fantasy.faith.player.concrete.HumanPlayer;
...
public class Chessboard extends View implements IChessboard{
	...
	//点大小
	private static int pointSize = 55;
	...
	public boolean onKeyDown(int keyCode, KeyEvent msg) {
		if (currentMode == READY && (keyCode == KeyEvent.KEYCODE_2 || keyCode == KeyEvent.KEYCODE_1)) {
			if(keyCode == KeyEvent.KEYCODE_2){//2键，人机对战
				player2 = computer;
			}else if(keyCode == KeyEvent.KEYCODE_1){//1键，人--人对战
				player2 = human;
			}
			restart();
			setMode(RUNNING);
		}else if(currentMode==RUNNING && keyCode == KeyEvent.KEYCODE_3){//重新开始
			restart();
			setMode(READY);
		}else{
			return false;
